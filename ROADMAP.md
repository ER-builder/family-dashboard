...
- ~~**Auto-return is too aggressive (2026-04-26).**~~ **Fixed 2026-04-26 with smarter approach (awaiting APK rebuild).** Three changes:
  - `AUTO_RETURN_MS` cut from 10 min → **5 min**.
  - Auto-return scheduling moved from `exitToPortalLauncher()` into the activity's `onStop` lifecycle hook. Fires for ALL backgrounding events: manual exit, **incoming calls**, screen-off. `onStart` cancels any pending alarm so a manual relaunch via `terry on` doesn't trip a stale alarm.
  - **Call-aware reschedule:** `AutoReturnReceiver.onReceive()` checks `AudioManager.getMode()` before launching. If `MODE_IN_COMMUNICATION` (VoIP / Portal video) or `MODE_IN_CALL` (cellular) is active, it reschedules itself for 5 min later instead of yanking the user out of the call. Net effect: dashboard returns ~5 min AFTER any call ends, never during.
  - Single 2s hold gesture restored (the dual-hold experiment was UX-confusing). Sublabel reads "back in 5m · pauses on calls".
  - Pending: APK rebuild via the recipe in 🔧 Pending Infra section.
