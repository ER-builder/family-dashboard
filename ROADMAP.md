...
3. ~~**🗓️ Family Countdown**~~ — **Framework shipped 2026-04-24, awaiting entries.** New "Countdowns" card in right column, hidden until you populate `COUNTDOWNS` array near the top of `index.html`. Format: `{ date: "YYYY-MM-DD", label: "Holiday in Greece", emoji: "🏖️", sub: "optional caption" }`. Shows next 3 within 90 days, sorted closest-first. Days-number is sage normally, ochre when ≤7 days, terracotta on the day. Past dates auto-pruned. *Original idea:* *"Holiday in 23 days 🏖️"*, *"Mum's birthday in 5 days 🎂"*.
...
- **Numbers in Fraunces are pretty but hard to read for young kids.** The serif tabular figures used everywhere big numbers appear (clock, weather temp, weather hi/lo, countdown days, transit arrival times) look editorial but the curvy serif strokes are taxing for early readers. Options: (a) swap the **clock** to JetBrains Mono (already loaded), keep Fraunces for everything else as a compromise — clock is the highest-priority number; (b) add a `--num-font` CSS variable defaulting to mono and swap selectively; (c) use Fraunces' "tnum" + "lnum" features but bump weight to 700 so digits are chunkier (Fraunces has plenty of weight range). Recommendation: try (a) first — single-line CSS change, see if kids find it easier — then escalate if needed.
