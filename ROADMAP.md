...
- **Auto-return is too aggressive (2026-04-26).** Elul exited the dashboard in the morning and wanted to stay in Portal mode all day for video calls; the kiosk kept relaunching every 10 minutes. Current `AUTO_RETURN_MS = 10L * 60L * 1000L`. Options: (a) **bump default to 1–2 hours** (least surgical, but still interrupts long sessions), (b) **two-tier hold gesture** — short 2s hold = 10 min auto-return, long 5s hold = "stay until midnight or next reboot" (sticky exit, no alarm), (c) **escalating timer** — first exit fires 10 min alarm, but if user exits AGAIN within 30 min the alarm doubles each time (10 → 30 → 2hr → none). Recommended: **option (b)** — explicit user intent for long-stay vs short-break, no surprise interruptions. Implementation lives in `MainActivity.kt:exitToPortalLauncher()` + a new gesture path in `index.html` for the long-hold.
