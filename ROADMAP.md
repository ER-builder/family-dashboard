...
- **Bus 102 countdown — refactor + diagnostic shipped 2026-04-24, awaiting Terry verification.** Rewrote `loadTflArrivals()` to sequential `for` loop (no nested `Promise.all`), defensive null-checks, no optional-chaining-on-array-index. Added a tiny diagnostic strip at the bottom of the arrivals card (`live=N sched=N err=N @HH:MM:SS`) so we can SEE on Terry whether the function runs and which branch fires. **Walk past Terry next time you're in the kitchen** and check what the dbg line says: if it shows `live≥1` or `sched≥3`, the rendering works and we just remove the dbg line. If it shows `err≥1`, network/CORS issue. If the dbg line itself isn't visible, JS is throwing earlier — open Chrome DevTools via `chrome://inspect` from your laptop to see the console. Priority: medium.
