...
- **Auto-return real fix (2026-04-26, awaiting APK rebuild).** Earlier "fix" (5-min timer + onStop scheduling + call-aware receiver) was correct in design but failed in practice — investigation showed the alarm fired 8 times reliably every 5 min, then stopped, but **none of the firings actually launched the dashboard** because `exitToPortalLauncher()` was disabling our HomeAlias before exit, killing our launcher status, which Android 10+ requires for `startActivity()` from a BroadcastReceiver background context. The HomeAlias-disable had been added in iter 4 to prevent snap-back when we were the registered default Home — but Aloha is now permanent default Home (user picked "Always"), so the disable is unnecessary AND harmful. **Fix:** removed the `setComponentEnabledSetting(DISABLED)` block from `exitToPortalLauncher()`. HomeAlias stays enabled across exits → launcher exemption preserved → background activity start works. Diagnosis: `dumpsys alarm | grep AUTO_RETURN` showed firing history; `dumpsys audio` confirmed call-mode transitions are clean. Pending: APK rebuild.
