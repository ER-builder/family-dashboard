...
- ~~**Auto-return is too aggressive (2026-04-26).**~~ **Fixed 2026-04-26 (awaiting APK rebuild).** Two changes: (1) `AUTO_RETURN_MS` cut from 10 min → **5 min** (Elul's preferred default for short breaks). (2) **Dual-stage hold** added on the EXIT pill: hold 2s and release = `kiosk://exit` → 5-min auto-return as before; hold 5s and release = `kiosk://exit?sticky=1` → no alarm, stays in Portal until next manual launch (`terry on`) or reboot. Visual: ring fills ochre 0–2s, switches to terracotta + refills 2–5s. Sublabel updates ("5m · keep holding for all day" → "Keep holding for ALL DAY…" → "Release for ALL-DAY exit"). New `cancelAutoReturn()` in MainActivity.kt clears any pending alarm when the sticky path fires (so a previously-scheduled return doesn't fire on top). Pending: build/install command from APK rebuild section.
